...
defineSupportCode(({Given, Then, When}) => {
  ...
  When(/^I login to Mercury using "([^"]*)" and "([^"]*)"$/, (userName, password) => {
    var home = client.page.home();
    return home.login(userName, password);
  });

  Then(/^Page titile is "([^"]*)"$/, (title) => {
    client.verify.title(title);
  });
  ...
  When(/^Confirm flight using first name as "([^"]*)", last name as "([^"]*)" and credit card number as "([^"]*)"$/, (firstName, lastName, creditNumber) => {
